...
這一章就是你第一次完整跑模組 S 的 AI 發包法:把活動企劃壓成 brief(**開規格**)→ 用 prompt 範本交給 AI(**下發包**)→ 對照驗收清單逐項檢查(**做驗收**)。下面的步驟就是這三步。
...
**任務**:挑你自己一個真實活動,用本章的四層 brief 生一個單檔 landing,存成 `index.html` 雙擊打開。
**預期成果**:打開看到 hero + 一顆主 CTA + 三個區塊;把視窗縮到手機寬度不破版、文字不溢出。
**卡關怎麼辦**:破版多半是回覆被截斷,跟 AI 說「從頭給我完整的單一 HTML 檔、手機優先,不要省略任何段落」。
